...
def about(request):
    return render(request,"GRsystem/about.html")

def login(request):
    return render(request,"GRsystem/login.html")

def admin(request):
    return render(request,"GRsystem/admin.html")
...
def  change_password(request):
		...
		if password_reset_form.is_valid():
			...
			if associated_users.exists():
				for user in associated_users:
					...
					email_template_name = "GRsystem/account_activation_email.html"
					c = {
					"email":user.email,
	...
	return render(request=request, template_name="GRsystem/change_password.html", context={"password_reset_form":password_reset_form})
...
class StudentListView(ListView):
        model = Complaint
        template_name = 'Grsystem/main.html'
...
#     Image = Complaint.objects.filter(id=cid).values('image')
...
#     p.drawString(25, 770,detailimage)
